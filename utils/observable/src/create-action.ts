...
export function createAsyncAction<
    PA1 extends PrepareAction<any>,
    PA2 extends PrepareAction<any>,
    T extends string = string
>(
    type: T,
    builder: { request: PA1; success: PA2 }
): {
    request: PayloadActionCreator<ReturnType<PA1>['payload'], `${T}::request`, PA1>;
    success: PayloadActionCreator<ReturnType<PA2>['payload'], `${T}::success`, PA2>;
};
...
>(
    type: T,
    builder: { request: PA1; failure: PA2 }
): {
    request: PayloadActionCreator<ReturnType<PA1>['payload'], `${T}::request`, PA1>;
    failure: PayloadActionCreator<ReturnType<PA2>['payload'], `${T}::failure`, PA2>;
};

export function createAsyncAction<
    PA1 extends PrepareAction<any>,
    PA2 extends PrepareAction<any>,
    PA3 extends PrepareAction<any>,
    T extends string = string
>(
    type: T,
    builder: { request: PA1; success: PA2; failure: PA3 }
): {
    request: PayloadActionCreator<ReturnType<PA1>['payload'], `${T}::request`, PA1>;
    success: PayloadActionCreator<ReturnType<PA2>['payload'], `${T}::success`, PA2>;
    failure: PayloadActionCreator<ReturnType<PA3>['payload'], `${T}::failure`, PA3>;
};
...
export function createAsyncAction(
    ...
    const actions = {
        request: createAction([type, 'request'].join('::'), builder.request),
    };
    if (builder.success) {
        Object.assign(actions, {
            success: createAction([type, 'success'].join('::'), builder.success),
        });
    }
    if (builder.failure) {
        Object.assign(actions, {
            success: createAction([type, 'failure'].join('::'), builder.failure),
        });
    }
    return actions;
}
